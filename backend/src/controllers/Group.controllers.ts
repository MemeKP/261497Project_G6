  ...
  try {
    ...
    if (!sessionId || !tableId) {
      return res.status(200).json({
        success: false,
        message: "Session ID and Table ID are required",
      });
    }
    ...
    if (!diningSession) {
      return res.status(200).json({
        success: false,
        message: "Invalid session or table ID, or session is not active",
      });
    }
    ...
    if (existingGroup) {
      return res.status(200).json({
        success: true,
        message: `Group for table ${tableId} already exists`,
        group: {
          id: existingGroup.id,
          tableId: existingGroup.table_id,
          createdAt: existingGroup.created_at,
    ...
    return res.status(201).json({
      ...
      group: {
        id: newGroup.id,
        tableId: newGroup.table_id,
        creatorUserId: newGroup.creator_user_id,
...
/**
 *  GET GROUP BY SESSION
 */
export const getGroupBySession = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { sessionId } = req.params;
    if (!sessionId) {
      return res.status(200).json({ success: false, message: "Session ID is required" });
    }
    ...
    if (!group) {
      return res.status(200).json({
        success: false,
        message: "Group not found for this session",
      });
    }
