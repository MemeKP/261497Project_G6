...
const CartPage = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [confirmItem, setConfirmItem] = useState<CartItem | null>(null);
  const [hasCheckedOutOrder, setHasCheckedOutOrder] = useState(false); // ✅ track only checked-out orders
  ...
  useEffect(() => {
    ...
    // ✅ Check if there’s any checked-out order in this session
    const checkCheckedOutOrders = async () => {
      try {
        ...
        const hasPaidOrder = Array.isArray(orders)
          ? orders.some(
              (o: any) => o.status && o.status !== "DRAFT" && o.status !== "CLOSED"
            )
          : false;

        setHasCheckedOutOrder(hasPaidOrder);
      } catch (err) {
        console.error("Error checking checked-out orders:", err);
      }
    };

    if (sessionId) {
      fetchCart();
      checkCheckedOutOrders();
    }
  }, [sessionId]);
          ...
          <button
            onClick={async () => {
              try {
                ...
                alert("Checkout successful! Redirecting to Order Status...");
                setCart([]);
                navigate(`/orderstatus/${sessionId}`);
              } catch (err) {
                console.error(err);
