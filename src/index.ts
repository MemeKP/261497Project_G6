...
import { users, admin, dining_sessions, groups, group_members, menu_items, orders, order_items } from "@db/schema.js";
...
app.post("/auth/register", async (req, res, next) => {
  try {
    ...
    let newUser;
    if (userType === 'admin') {
      newUser = await dbClient.insert(admin).values({
        name,
        email,
      ...
      newUser = await dbClient.insert(users).values({
        name,
        email,
        password: hashedPassword,
      }).returning({
...
app.post("/dining_session/start", requireAdmin, async (req, res, next) => {
  try {
    ...
    const newSession = await dbClient.insert(dining_sessions).values({
      table_id: tableId,
    ...
    const qrData = {
      ...
      tableId: tableId,
      url: `${process.env.CLIENT_URL || 'http://localhost:3000'}/table/${newSession[0].id}`
    };
...
app.post("/dining_session/end", requireAdmin, async (req, res, next) => {
  try {
    ...
    const updatedSession = await dbClient.update(dining_sessions)
      .set({
        ended_at: endedAt,
        status: 'COMPLETED',
    ...
    const duration = endedAt.getTime() - (activeSession.started_at?.getTime() || endedAt.getTime());
...
app.post("/group/create", async (req, res, next) => {
  try {
    ...
    const newGroup = await dbClient.insert(groups).values({
      table_id: tableId,
...
app.post("/group_members/add", async (req, res, next) => {
  try {
    ...
    const newMember = await dbClient.insert(group_members).values({
      name,
      group_id: groupId,
      user_id: userId || null,
      note: note || null
...
app.delete("/group_members/:groupId", async (req, res, next) => {
  try {
    ...
    await dbClient.delete(group_members).where(
...
app.get("/orders", requireAdmin, async (req, res, next) => {
  try {
    const { status, tableId, diningSessionId, limit = 50, offset = 0 } = req.query;
    ...
    if (diningSessionId && !isNaN(Number(diningSessionId))) {
      orderWhereConditions.push(eq(orders.dining_session_id, Number(diningSessionId)));
      orderWhereConditions.push(isNotNull(orders.dining_session_id));
    }

    const allOrders = await dbClient.query.orders.findMany({
      where: orderWhereConditions.length > 0 ? and(...orderWhereConditions) : undefined,
    ...
    res.json({
      ...
      statistics: {
        ...
        statusCounts,
        totalItems: ordersWithItems.reduce((sum, order) => 
...
app.put("/orders/:orderId/status", requireAdmin, async (req, res, next) => {
  try {
    ...
    const updatedOrder = await dbClient.update(orders)
      .set({ status })
      .where(eq(orders.id, orderId))
      .returning({
        id: orders.id,
